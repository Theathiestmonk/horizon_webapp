  const CFG = {
    ...
    invoiceCodeCell:        'F17',
    webhookUrl: 'https://webshots-filme-pipes-legends.trycloudflare.com/api/v1/invoices/',
  };
  ...
  // function below should read the webcook base through this instead of
  ...
  // Per-vehicle columns N-Q. All 4 are optional per row. district_origin_code
  // / state_origin_code feed CHA CI's per-model block (see
  // buildChaCiVehicleBlock_ below), falling back to the Products-tab
  // per-model value when blank. eic_cert_no / first_registration_date are
  // collected here but currently unused by any description generator — CI's
  // description now prints Products!O exactly as typed (see the
  // items.forEach note near description_commercial below) rather than an
  // auto-generated per-vehicle block, so these two columns have no active
  // consumer right now.
  ...
  // accessories still feeds buildCommercialInvoiceTrailer_'s "FOLLOWING
  // ACCESSORIES ARE INSTALLED..." line. engine_cc/make are collected here
  // but have no active consumer right now — they used to feed the
  // auto-generated per-vehicle Commercial Invoice/Packing List blocks,
  // which were replaced by plain Products!O / Products!H text (see the
  // items.forEach note near description_commercial). Appended after
  // Products' own existing columns rather than at a fixed position, since
  // Products' width varies per sheet and is user-maintained, not created by
  // this script the way Stock is.
  ...
  //   2. <chassis2>  <color2>
  //   ...
  // Chassis + colour share ONE line (not two) — halves the vertical space
  // this block needs per vehicle, which is what actually determines how
  // many vehicles fit on PI FORMAT's single page before font size has to
  // shrink (confirmed by testing: at the old two-line format, ~5 vehicles
  // was the ceiling at 11pt; row/margin padding had already been fully
  // reclaimed elsewhere, so this per-vehicle format was the only remaining
  // lever). Only vehicles that actually have a chassis_no get a numbered
  // line — a model group with none assigned yet (e.g. still at the "target
  // quantity" stage, no real Stock rows linked) falls through unchanged,
  // same plain single-line description PI FORMAT has always shown. A group
  // with a MIX (some vehicles chassis-assigned, some not) lists only the
  // assigned ones — dynamic per-vehicle, not an all-or-nothing switch for
  // the whole group.
  ...
  //   <base CI description — Products!O, printed exactly as typed>
  //   1. <chassis1>
  //   2. <chassis2>
  //   ...
  // Chassis number only — no colour — one per line, so a model group with
  // several distinct vehicles (e.g. 4 units of the same model, each a
  // different chassis) reads as a clean numbered list under the shared
  // description instead of a single line that can't represent more than one
  // vehicle. Same "only vehicles with a chassis_no assigned get a numbered
  // line" behavior as buildPiVehicleBlock_ above — falls through unchanged
  // when nothing in the group has one yet.
  function buildCommercialInvoiceVehicleBlock_(baseDesc, vehicleGroup) {
    var withChassis = vehicleGroup.filter(function(v) { return String(v.chassis_no || '').trim(); });
    if (withChassis.length === 0) return baseDesc;
    var lines = [baseDesc];
    withChassis.forEach(function(v, i) {
  ...
  //   <base PL description — Products!H, printed exactly as typed>
  //   1.    <chassis1> <color1>
  //   2.    <chassis2> <color2>
  //   ...
  // Chassis + colour on one line, same as PI FORMAT, but with FOUR spaces
  // between the number and the chassis value (not one) — client-specified
  // exact spacing for readability in the Packing List's wider description
  // column. Same chassis-assigned-only / unchanged-fallthrough behavior as
  ...
  function buildPayload(ss, ctrl, inv) {
    ...
    // pref_trade_code (col D) column is no longer read at all; col T (index 19),
    // formerly invoice_code (also dead — see above), is now reused for the
    // Annexure C item 15 "Vehicles" per-model description (see descAnxC below).
    ...
    // 14=desc_commercial_invoice, 15=desc_scomet, 16=engine_cc, 17=make, 18=accessories, 19=ANX C Description, 20=stock_model_name
    ...
    vehicles.forEach(function(v) {
      ...
      // ANX C Description (col T, index 19) — falls back to the plain model
      // name (not fallbackDesc) so a product row that hasn't had column T
      // filled in yet still reproduces today's "just the product name"
      // behavior for Annexure C item 15, exactly as before this column existed.
      var descAnxC      = prod ? (String(prod[19]).trim() || modelDisplay) : modelDisplay;  // ANX C Description
      var productPrice  = 0;
      ...
      if (!itemsObj[groupKey]) {
        itemsObj[groupKey] = {
          ...
          description_anx_c:      descAnxC,  // Products!T — falls back to the plain model name; see buildAnxCVehiclesList_ below
          quantity:               0,
          rate_per_unit:          price,
          amount_usd:             0,
    ...
    items.forEach(function(it, idx) {
      ...
      // Commercial Invoice / Packing List descriptions start from EXACTLY
      // what's typed on Products!O (desc_commercial_invoice) / Products!H
      // (desc_packing_list) — no model name appended, same "printed exactly
      // as typed" base text as before — but now get the numbered per-vehicle
      // chassis breakdown appended underneath (chassis only for CI, chassis
      // + colour for PL), same chassis-assigned-only fallthrough as PI
      // FORMAT's block above. See buildCommercialInvoiceVehicleBlock_ /
      // buildPackingListVehicleBlock_ for the exact per-line formats.
      it.description_commercial = buildCommercialInvoiceVehicleBlock_(it.description_commercial, it.vehicle_list);
      ...
      if (idx === items.length - 1) {
        ...
        // Roman-numeral labels (YEAR OF MANUFACTURE, LC NO., etc.) always
        // start fresh at "I." now — nothing before this trailer prints any
        // Roman-numeral lines of its own anymore.
        var trailer = buildCommercialInvoiceTrailer_(ctrl, accessories, resolvedPiInvoiceNo || inv, resolvedPiInvoiceDate || invoiceDateStr, countryOfOriginVal, fallbackYear, 0);
